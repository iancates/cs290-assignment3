...
bar = function (doubleArray) {
    var count;
    for (count = 0; count < doubleArray.length; count += 1) {
        doubleArray[count] = doubleArray[count] * 2;     
        if (isNaN(doubleArray[count])) {
            return false;   
        }
    };
    return true;
...
function parseGit(logArray) {
    ...
    var count;
    for (count = 0; count < logArray.length; count += 1) {
        newGitLog[count] = new GitLog(); //Make this part of the array a GitLog so we can use hash, date, and message
        newGitLog[count].hash = logArray[count].split(" ")[0];
        newGitLog[count].date = new Date(logArray[count].substring(logArray[count].indexOf(' ') + 1, logArray[count].indexOf('"')));//Use substring to pick out the parts I need
        newGitLog[count].message = logArray[count].substring(logArray[count].indexOf('"') + 1, logArray[count].lastIndexOf('"'));//Use "" to find the message
    }
    return newGitLog;
}
//end your code
